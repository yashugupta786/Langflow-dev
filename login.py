...
class GateLoginInput(BaseModel):
    email: str
    token: str

@router.post("/gate_login")
async def gate_login(
    payload: GateLoginInput,
    response: Response,
    db: DbSession,
):
    """
    SSO-mimic login: accepts email and a token (random in dev, JWT in prod).
    """
    email = payload.email.strip().lower()

    # In dev: accept any token (not empty)
    if not payload.token or not isinstance(payload.token, str):
        raise HTTPException(status_code=401, detail="Missing or invalid SSO token.")

    # In prod: validate JWT here.
